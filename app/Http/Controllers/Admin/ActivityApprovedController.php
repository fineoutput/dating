...
class ActivityApprovedController extends Controller
{
    /**
    ...
    public function approvedactivity()
    {
        $data = Activity::orderBy('id','DESC')->where('status',2)->get(); // Fetch all users
        $title = "Approved Activity";
        return view('admin.activityapproved.approvedactivity', compact('data', 'title'));
    }
